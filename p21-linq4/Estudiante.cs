class Estudiante
{
    ...
    public override string ToString () => $"Matricula {Matricula, 6} Nombre: {Nombre, -18} Domicilio: {Domicilio, -15} Municipio: {Municipio, -10} Sexo: {(Sexo == 'H' ? "Hombre" : "Mujer")} Edad: {Edad, 4} Becado: {(Becado?"Becado":"No becado")} Califs: {string.Join(",", Califs)}";
}